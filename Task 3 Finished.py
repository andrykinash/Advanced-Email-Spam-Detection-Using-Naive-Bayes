...
# Changed to bag of words for task 3
def bag_of_words(emails, vocabulary):
    vectors = []
    vocab_list = list(vocabulary)
    for email in emails:
        email_word_count = Counter(email)
        vector = [email_word_count[word] for word in vocab_list]
...
def train_naive_bayes(train_vectors, train_labels):
    ...
    word_prob_spam = [(sum(word_vector[i] for i, label in enumerate(train_labels) if label == 1) + 1) / 
                      (sum(sum(train_vectors[i]) for i, label in enumerate(train_labels) if label == 1) + len(vocabulary))
                      for word_vector in zip(*train_vectors)]
    word_prob_normal = [(sum(word_vector[i] for i, label in enumerate(train_labels) if label == 0) + 1) / 
                        (sum(sum(train_vectors[i]) for i, label in enumerate(train_labels) if label == 0) + len(vocabulary))
                        for word_vector in zip(*train_vectors)]
